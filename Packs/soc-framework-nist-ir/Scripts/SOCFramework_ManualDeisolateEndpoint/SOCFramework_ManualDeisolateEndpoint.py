...
def main():
    ...
    if not endpoint_id:
        return_error(
            "SOCFramework_ManualDeisolateEndpoint: endpoint ID not found. "
            "Ensure SOCFramework.Primary.Endpoint is set in context."
        )
    result = demisto.executeCommand(
        "SOCCommandWrapper",
        {
            "action": "soc-deisolate-endpoint",
    ...
    if is_error(result):
        return_error(
            f"SOCFramework_ManualDeisolateEndpoint: {get_error(result)}"
        )
    return_results(CommandResults(
        readable_output=(
            f"De-isolate request submitted for: `{endpoint_id}`\n"
