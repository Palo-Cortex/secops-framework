...
#
# Verifies XSIAM API endpoints are responding AND that the correlation
# rule write path works. Tests the exact code path that upload uses:
#   1. GET correlations — verify 200 + parseable response
...
#
# Usage:
#   bash tools/platform_health_check.sh
#   source tools/platform_health_check.sh && check_platform_health
#
...
check_platform_health() {
    ...
    # ── Step 2: CREATE test rule — write path ─────────────────────────────────
    if [ "$failed" -eq 0 ]; then
        local create_body
        create_body=$(cat <<EOF
{"request_data": [{"name": "${_HEALTH_TEST_RULE_NAME}", "severity": "SEV_010_INFO", "xql_query": "dataset = xdr_data | filter event_type = ENUM.PROCESS | fields agent_hostname", "is_enabled": "false", "description": "Health check probe — safe to delete", "alert_name": "Health Check", "alert_category": "OTHER", "alert_description": "health check", "alert_fields": {}, "execution_mode": "SCHEDULED", "search_window": "10 minutes", "simple_schedule": "10 minutes", "timezone": "UTC", "crontab": "*/10 * * * *", "suppression_enabled": false, "suppression_duration": null, "suppression_fields": null, "dataset": "alerts", "user_defined_severity": null, "user_defined_category": null, "mitre_defs": {}, "investigation_query_link": null, "drilldown_query_timeframe": "ALERT", "mapping_strategy": "AUTO"}]}
EOF
        )

        response=$(curl -s -w "\n%{http_code}" \
            -X POST "${DEMISTO_BASE_URL}/public_api/v1/correlations/create_or_update" \
            -H "Authorization: ${DEMISTO_API_KEY}" \
            -H "x-xdr-auth-id: ${XSIAM_AUTH_ID}" \
            -H "Content-Type: application/json" \
            -d "$create_body" \
        ...
        if [ "$http_code" = "200" ]; then
            ...
            # Extract rule_id from the create response
            local rule_id
    ...
    if [ "$failed" -gt 0 ]; then
        ...
        echo "  ✓ Platform healthy — read and write paths verified"
        echo ""
        return 0
    fi
}
